...
insert_button = tk.Button(root, text="Insert", command=insert_user)
...
def insert_address():
    Address_ID=Address_entry.get()
    City = City_entry.get()
    District=District_entry.get()
    Neighborhood = Neighborhood_entry.get()
   

    query = "INSERT INTO Address (Address_ID,City,District,Neighborhood) VALUES (%d,%s,%s, %s)"
...
District_entry.grid(row=2,column=0)
...
new_Neighborhood_label.grid(row=6, column=0)
new_Neighborhood_entry.grid(row=6, column=1)
