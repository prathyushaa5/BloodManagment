...
def choice():
    ...
    d_button.grid(row=0,column=0,columnspan=2)
    d_button=tk.Button(root,text="Recipient",command=recipient)
    d_button.grid(row=0,column=1,columnspan=2)
...
# Update data in Donor table
    def update_donor():
      ...
      if count1==0:
          ...
          cursor.execute("Select COUNT(*) from Blood_Type where Blood_ID=%s",(new_Blood_ID,))
          count2=cursor.fetchone()[0]
          if count2==0:
           messagebox.showinfo("Enter a valid Blood_ID!")
  ...
  def update_recipient():
    ...
    if count1==0:
           ...
           if count3==0:
             ...
             query = "UPDATE Recipient SET First_Name = %s, Last_Name = %s,Blood_ID=%s,Address_ID=%s,Phone_No=%s WHERE Recipient_ID = %s"
             values = (new_First_Name, new_Last_Name,new_Blood_ID,new_Address_ID,new_Phone_No,Recipient_ID)
             cursor.execute(query, values)
             db.commit()
...
# Labels and Entry fields for Recipient
  ...
  new_First_Name_label = tk.Label(root, text="New First Name:")
  new_Last_Name_label = tk.Label(root, text="New Last Name:")
  new_Blood_ID_label = tk.Label(root, text="New Blood_ID:")
  new_Address_ID_label = tk.Label(root, text="New Address_ID:")
...
# Grid layout for Recipient
  ...
  new_Blood_ID_label(row=9,column=0)
  new_Blood_ID_entry(row=9,column=1)
  new_Address_ID_label(row=10,column=0)
  new_Address_ID_entry(row=10,column=1)
  new_Phone_No_label(row=11,column=0)
  new_Phone_No_entry(row=11,column=1)
  update_button.grid(row=12, columnspan=2)
  delete_button.grid(row=12, columnspan=2)
  display_button.grid(row=13, columnspan=2)
  result_text.grid(row=14, column=0, padx=10, pady=10)
  previous_button.grid(row=15,column=0,columnspan=1)
  next_button.grid(row=15,column=1,columnspan=1)
