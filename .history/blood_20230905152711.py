...
def option():
    ...
    bg_image=PhotoImage(file="111.png")
    background_label=tk.Label(root,image=bg_image)
    background_label.place(relwidth=1,relheight=1)
    frame=tk.Frame(root,bg="white")
    frame.place(relx=0.5,rely=0.5,anchor="center")
    heading_label=tk.Label(root,text="MENU",font=("Comic Sans MS",12,"bold"),bg="white",fg="black")
    heading_label.pack(pady=20)
    a_button=tk.Button(frame,text="Address ",command=address,fg="white",bg="blue")
    bt_button=tk.Button(frame,text="BloodType",command=blood_type,fg="white",bg="blue")
    ...
    b_button=tk.Button(frame,text="Blood_Bank ",command=blood_bank,fg="white",bg="blue")
    ...
    bt_button.grid(row=2,column=2,padx=10,pady=10)
    d_button.grid(row=3,column=2,padx=10,pady=10)
    r_button.grid(row=4,column=2,padx=10,pady=10)
    b_button.grid(row=5,column=2,padx=10,pady=10)
    g_button.grid(row=6,column=2,padx=10,pady=10)
    t_button.grid(row=7,column=2,padx=10,pady=10)
...
def address():
   def insert_address():
       ...
       if not Address_ID:
         messagebox.showinfo("Error","Cannot be empty")
       else:
         cursor.execute("Select COUNT(*) from Address where Address_ID=%s",(Address_ID,))
         count1=cursor.fetchone()[0]
         if count1>0:
           messagebox.showinfo("Error","Address_ID already taken")
         else:    
   ...
   def update_address():
       ...
       count1=cursor.fetchone()[0]
       if count1==0:
           messagebox.showinfo("Error","Enter a valid Address_ID to update!")
       else:    
...
# Delete data from Address table
   def delete_address():
       ...
       count1=cursor.fetchone()[0]
       if count1==0:
           messagebox.showinfo("Error","Enter a valid Address_ID to delete!")
       else:    
   ...
   def display_address():
      ...
      for result in results:
        display_text.insert(tk.END, f"Address_ID: {result[0]}\n")
...
# Create the GUI window
   ...
   window_height=600
   root.geometry(f"{window_width}x{window_height}")
   root.configure(bg="#013a63")
   heading_label=tk.Label(root,text="ADDRESS",font=("Serif",15,"bold"),bg="#013a63",fg="WHITE")
   heading_label.pack(pady=20)
   frame=tk.Frame(root,borderwidth=5,relief="flat",highlightthickness=0.5,bg="#caf0f8")
   frame.place(relx=0.5,rely=0.5,anchor="center")
  

# Labels and Entry fields for Address
   Address_ID_label = tk.Label(frame, text="Address ID:",bg="#caf0f8")
   City_label = tk.Label(frame, text="City:",bg="#caf0f8")
   District_label = tk.Label(frame, text="District:",bg="#caf0f8")
   Neighborhood_label = tk.Label(frame, text="Neighborhood:",bg="#caf0f8")
   Address_ID_entry = tk.Entry(frame)
   City_entry = tk.Entry(frame)
   District_entry=tk.Entry(frame)
   Neighborhood_entry = tk.Entry(frame)
   new_City_label = tk.Label(frame, text="New City:",bg="#caf0f8")
   new_District_label = tk.Label(frame, text="New District:",bg="#caf0f8")
   new_Neighborhood_label = tk.Label(frame, text="New Neighborhood:",bg="#caf0f8")
...
def blood_type():
   ...
   root.config(bg="#013a63")
   window_width=600
   window_height=600
   frame=tk.Frame(root,borderwidth=1,relief="flat",highlightthickness=0.5,highlightbackground="white",bg="#caf0f8")
   frame.place(relx=0.5,rely=0.5,anchor="center")
   root.geometry(f"{window_width}x{window_height}")
   heading_label=tk.Label(root,text="BLOOD TYPE",font=("Serif",15,"bold"),bg="#013a63",fg="WHITE")
   heading_label.pack(pady=20)
...
def donor():
    def insert_donor():
     ...
     if not Donor_ID:
        ...
        count1=cursor.fetchone()[0]
        if count1==0:
         messagebox.showinfo("Error","Insert a valid Blood_ID")
        cursor.execute("SELECT COUNT(*) FROM Address WHERE Address_ID=%s",(Address_ID,))    
        count2=cursor.fetchone()[0]
        if count2==0:
         messagebox.showerror("Error","Insert a valid Address_ID")
        if count1>0 and count2>0:
         cursor.execute("Select COUNT(*) from Donor where Donor_ID=%s",(Donor_ID,))
         count3=cursor.fetchone()[0]
         if count3>0:
           messagebox.showinfo("Error","Donor_ID already taken!")
         else:        
           query = "INSERT INTO Donor (Donor_ID,First_Name, Last_Name, Blood_ID, Address_ID,Phone_No) VALUES (%s,%s, %s, %s, %s,%s)"
           values = (Donor_ID,First_Name, Last_Name, Blood_ID,Address_ID,Phone_No)
           cursor.execute(query, values)
           db.commit()
...
# Update data in Donor table
    def update_donor():
      ...
      new_Phone_No=new_Phone_No_entry.get()
      cursor.execute("Select COUNT(*) from Donor where Donor_ID=%s",(Donor_ID,))
      count1=cursor.fetchone()[0]
      if count1==0:
           messagebox.showinfo("Error","Enter a valid Donor_ID to update!")
      else:
          cursor.execute("Select COUNT(*) from Address where Address_ID=%s",(new_Address_ID,))
          count3=cursor.fetchone()[0]
          if count3==0:
           messagebox.showinfo("Error","Enter a valid Address_ID!")
          else:       
           query = "UPDATE Donor SET First_Name = %s, Last_Name = %s,Address_ID=%s,Phone_No=%s WHERE Donor_ID = %s"
           values = (new_First_Name, new_Last_Name,new_Address_ID,new_Phone_No,Donor_ID)
           cursor.execute(query, values)
           db.commit()
...
# Delete data from Donor table
    def delete_donor():
      ...
      count1=cursor.fetchone()[0]
      if count1==0:
           messagebox.showinfo("Error","Enter a valid Donor_ID to delete!")
      else:     
    ...
    def display_donor_details():
      ...
      for donor in donor_data:
        result_text.insert(tk.END, f"ID: {donor[0]}, First Name: {donor[1]}, Last Name: {donor[2]}, Blood ID: {donor[3]}, Address ID: {donor[4]},Phone_No:{donor[5]}\n")    
   ...
   # Create the GUI window
    ...
    window_height=700
    root.geometry(f"{window_width}x{window_height}")
    frame=tk.Frame(root,borderwidth=5,relief="flat",highlightthickness=0.5,highlightbackground="white",bg="")
    frame.place(relx=0.5,rely=0.5,anchor="center")
    heading_label=tk.Label(root,text="DONOR DETAILS",font=("Serif",15,"bold"),bg="#013a63",fg="WHITE")
    heading_label.pack(pady=20)
    

# Labels and Entry fields for Donor
    Donor_ID_label = tk.Label(frame, text="Donor ID:",bg="#caf0f8")
    First_Name_label = tk.Label(frame, text="First Name:",bg="#caf0f8")
    Last_Name_label = tk.Label(frame, text="Last Name:",bg="#caf0f8")
    Blood_ID_label = tk.Label(frame, text="Blood ID:",bg="#caf0f8")
    Address_ID_label = tk.Label(frame, text="Address ID:",bg="#caf0f8")
    Phone_No_label=tk.Label(frame,text="Phone_No:",bg="#caf0f8")
    ...
    new_First_Name_label = tk.Label(frame, text="New First Name:",bg="#caf0f8")
    new_Last_Name_label = tk.Label(frame, text="New Last Name:",bg="#caf0f8")
    new_First_Name_label = tk.Label(frame, text="New First Name:",bg="#caf0f8")
    new_Address_ID_label = tk.Label(frame, text="New Address_ID:",bg="##caf0f8")
    new_Phone_No_label=tk.Label(frame,text="New Phone_No:")
...
# Grid layout for Donor
    ...
    insert_button.grid(row=6,column=1,columnspan=3)
    ...
    display_button.grid(row=13,column=1,columnspan=3)
    result_text.grid(row=14)
    # previous_button.grid(row=14,column=0,columnspan=1)
    done_button.grid(row=15,column=1)
...
def recipient():
  def insert_recipient():
    ...
    if not Recipient_ID:
         ...
         count2=cursor.fetchone()[0]
         if count2==0:
          messagebox.showinfo("Error","Insert a valid Address_ID")
         if count1>0 and count2>0:
          cursor.execute("Select COUNT(*) from Recipent where Recipient_ID=%s",(Recipient_ID,))
          count3=cursor.fetchone()[0]
          if count3>0:
            messagebox.showinfo("Erro","Recipient_ID already taken!")
          else:   
...
# Update data in Recipient table
  def update_recipient():
    ...
    count1=cursor.fetchone()[0]
    if count1==0:
           messagebox.showinfo("Error","Enter a valid Recipient_ID to update!")
    cursor.execute("Select COUNT(*) from Address where Address_ID=%s",(new_Address_ID,))
    count3=cursor.fetchone()[0]
    if count3==0:
      messagebox.showinfo("Error","Enter a valid Address_ID!")
    else:
      query = "UPDATE Recipent SET First_Name = %s, Last_Name = %s,Address_ID=%s,Phone_No=%s WHERE Recipient_ID = %s"
...
# Delete data from Recipient table
  def delete_recipient():
    ...
    count1=cursor.fetchone()[0]
    if count1==0:
           messagebox.showinfo("Error","Enter a valid Recipient_ID to delete!")
    else:     
...
# Create the GUI window
  ...
  window_height=700
  root.config(bg="#013a63")
  frame=tk.Frame(root,borderwidth=5,relief="flat",highlightthickness=0.5,highlightbackground="white",bg="#caf0f8")
  frame.place(relx=0.5,rely=0.5,anchor="center")
  root.geometry(f"{window_width}x{window_height}")
  heading_label=tk.Label(root,text="RECIPIENT DETAILS",font=("Serif",15,"bold"),bg="#013a63",fg="WHITE")
  heading_label.pack(pady=20)

# Labels and Entry fields for Recipient
  Recipient_ID_label = tk.Label(frame, text="Recipient ID:",bg="#caf0f8")
  First_Name_label = tk.Label(frame, text="First Name:",bg="#caf0f8")
  Last_Name_label = tk.Label(frame, text="Last Name:",bg="#caf0f8")
  Blood_ID_label = tk.Label(frame, text="Blood ID:",bg="#caf0f8")
  Address_ID_label = tk.Label(frame, text="Address ID:",bg="#caf0f8")
  Phone_No_label = tk.Label(frame, text="Phone No:",bg="#caf0f8")
  ...
  new_Last_Name_label = tk.Label(frame, text="New Last Name:",bg="#caf0f8")
  new_Address_ID_label = tk.Label(frame, text="New Address_ID:",bg="#caf0f8")
  new_Phone_No_label = tk.Label(frame, text="New Phone_No:",bg="#caf0f8")
  new_First_Name_entry = tk.Entry(frame)
  new_Last_Name_entry = tk.Entry(frame)
  new_Address_ID_entry = tk.Entry(frame)
  new_Phone_No_entry = tk.Entry(frame)
...
# Insert data into Blood_bank table
  def insert_blood_bank():
    ...
    if not Bank_ID:
        ...
        count1=cursor.fetchone()[0]
        if count1>0:
          messagebox.showinfo("Error","Bank_ID already taken:")
        else: 
...
# Delete data from Blood_bank table
  def delete_blood_bank():
    ...
    count2=cursor.fetchone()[0]
    if count2==0:
      messagebox.showinfo("Error","Enter a valid Bank_ID")
    else: 
...
# Create the GUI window
  ...
  frame.place(relx=0.5,rely=0.5,anchor="center")
  root.geometry(f"{window_width}x{window_height}")
  heading_label=tk.Label(root,text="BLOOD BANK",font=("Serif",15,"bold"),bg="#013a63",fg="WHITE")
  heading_label.pack(pady=20)

# Labels and Entry fields for Blood_bank
  Bank_ID_label = tk.Label(frame, text="Bank ID:",bg="#caf0f8")
  Name_label = tk.Label(frame, text="Name:",bg="#caf0f8")
  Capacity_label = tk.Label(frame, text="Capacity:",bg="#caf0f8")
  Bank_ID_entry = tk.Entry(frame,bg="#caf0f8")
  Name_entry = tk.Entry(frame,bg="#caf0f8")
  Capacity_entry = tk.Entry(frame,bg="#caf0f8")
...
def gives_to():
  ...
  def insert_gives_to():
    ...
    if not Donation_ID:
        ...
        count1=cursor.fetchone()[0]
        if count1>0:
          messagebox.showinfo("Error","Donation_ID already taken")
        else:
          cursor.execute("Select COUNT(*) from Donor where Donor_ID=%s",(Donor_ID,))
          count2=cursor.fetchone()[0]
          if count2==0:
...
# Delete data from gives_to table
  def delete_gives_to():
    ...
    count1=cursor.fetchone()[0]
    if count1==0:
      messagebox.showinfo("Error","Enter a valid Donation_ID")
    else:  
...
# Create the GUI window
  ...
  frame=tk.Frame(root,bg="#caf0f8")
  frame.place(relx=0.5,rely=0.5,anchor="center")
  heading_label=tk.Label(root,text="GIVES TO",font=("Serif",15,"bold"),bg="#013a63",fg="WHITE")
  heading_label.pack(pady=20)
# Labels and Entry fields for gives_to
  Donation_ID_label = tk.Label(frame, text="Donation ID:")
  Donor_ID_label = tk.Label(frame, text="Donor ID:")
  Bank_ID_label = tk.Label(frame, text="Bank ID:")
  Amount_label = tk.Label(frame, text="Amount:")
  new_Amount_label = tk.Label(frame, text="New Amount (for update):")
  Donation_ID_entry = tk.Entry(frame)
  Donor_ID_entry = tk.Entry(frame)
  Bank_ID_entry = tk.Entry(frame)
  Amount_entry = tk.Entry(frame)
  new_Amount_entry = tk.Entry(frame)

# Buttons for gives_to
  insert_button = tk.Button(frame, text="Insert Data", command=insert_gives_to)
  delete_button = tk.Button(frame, text="Delete Data", command=delete_gives_to)
  display_button = tk.Button(frame, text="Display Data", command=display_gives_to)
  update_button = tk.Button(frame, text="Update Data", command=update_gives_to)
  next_button=tk.Button(frame,text="Next",command=next)
  # previous_button=tk.Button(root,text="prev",command=previous)
# Text widget for display
  display_text = tk.Text(frame, height=10, width=40, state=tk.DISABLED)
...
# Grid layout for gives_to
  ...
  display_button.grid(row=7,column=1)
  update_button.grid(row=8,column=1)
  display_text.grid()
  # previous_button.grid(row=10,column=0,columnspan=2)
  next_button.grid(row=10,column=1)
...
def takes_from():
  ...
  def insert_takes_from():
    ...
    if not Transfer_ID:
         ...
         count1=cursor.fetchone()[0]
         if count1>0:
           messagebox.showinfo("Error","Transfer_ID already taken")
         else:
           cursor.execute("Select COUNT(*) from Recipent where Recipient_ID=%s",(Recipient_ID,))
           count2=cursor.fetchone()[0]
           if count2==0:
             messagebox.showinfo("Error","Enter a valid Recipient_ID")   
           cursor.execute("Select COUNT(*) from Blood_Bank where Bank_ID=%s",(Bank_ID,))
           count3=cursor.fetchone()[0]
           if count3==0:
             messagebox.showinfo("Error","Enter a valid Bank_ID")
...
# Delete data from Takes_from table
  def delete_takes_from():
    ...
    count1=cursor.fetchone()[0]
    if count1==0:
      messagebox.showinfo("Error","Enter a valid Transfer_ID")
    else:  
...
# Create the GUI window
  ...
  frame=tk.Frame(root,bg="#caf0f8")
  frame.place(relx=0.5,rely=0.5,anchor="center")
  heading_label=tk.Label(root,text="TAKES FROM",font=("Serif",15,"bold"),bg="#013a63",fg="WHITE")
  heading_label.pack(pady=20)
# Labels and Entry fields for Takes_from
  Transfer_ID_label = tk.Label(frame, text="Transfer ID:",bg="#caf0f8")
  Recipient_ID_label = tk.Label(frame, text="Recipient ID:",bg="#caf0f8")
  Bank_ID_label = tk.Label(frame, text="Bank ID:",bg="#caf0f8")
  Amount_label = tk.Label(frame, text="Amount:",bg="#caf0f8")
  Transfer_ID_entry = tk.Entry(frame)
  Recipient_ID_entry = tk.Entry(frame)
  Bank_ID_entry = tk.Entry(frame)
  Amount_entry = tk.Entry(frame)
  new_Amount_label = tk.Label(frame, text="New Amount:",bg="#caf0f8")
  new_Amount_entry = tk.Entry(frame,bg="#caf0f8")

# Buttons for Takes_from
  insert_button = tk.Button(frame, text="Insert", command=insert_takes_from,bg="#caf0f8")
  delete_button = tk.Button(frame, text="Delete", command=delete_takes_from,bg="#caf0f8")
  display_button = tk.Button(frame, text="Display", command=display_takes_from,bg="#caf0f8")
  update_button = tk.Button(frame, text="Update", command=update_takes_from,bg="#caf0f8")
  next_button=tk.Button(frame,text="Next",command=next,bg="#caf0f8")

# Text widget to display records
  display_text = tk.Text(frame, height=10, width=50)
  display_text.config(state=tk.DISABLED)
