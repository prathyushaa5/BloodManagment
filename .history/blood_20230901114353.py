...
def option():
    ...
    window_height=400
    frame=tk.Frame(root)
    frame.place(relx=0.5,rely=0.5,anchor="center",background="gray")
    ...
    t_button.grid(row=0,column=0)
    a_button.grid(row=0,column=1)
    b_button.grid(row=1,column=0)
    d_button.grid(row=1,column=1)
...
# Grid layout for Address
   Address_ID_label.grid(side="left")
   Address_ID_entry.pack(side="right")
   ...
   new_City_label.pack()
   new_City_entry.pack()
   new_District_label.pack()
   new_District_entry.pack()
   new_Neighborhood_label.pack()
   new_Neighborhood_entry.pack()
   update_button.pack()
   delete_button.pack()
   next_button.pack()
   root.mainloop()
