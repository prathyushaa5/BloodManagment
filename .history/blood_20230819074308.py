...
def display_blood_type():
    ...
    for row in results:
        display_text.insert(tk.END, f"Blood_ID: {row[0]}\n")
        display_text.insert(tk.END, f"Blood_Code: {row[1]}\n")
        display_text.insert(tk.END, f"Donates_To: {row[2]}\n")
        display_text.insert(tk.END, f"Receives_From: {row[3]}\n\n")

# Create the GUI window
root = tk.Tk()
root.title("Blood Type Display")

# Text widget for displaying Blood_Type table
display_text = tk.Text(root, height=10, width=40)
display_text.pack()

# Button to display Blood_Type table
